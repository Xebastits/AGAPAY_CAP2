...
const CollapsibleDescription = ({ text }: { text: string }) => {
    ...
    return (
        <div className="bg-slate-50 p-3 rounded mb-4 w-full">
            <p className={`text-slate-600 break-words whitespace-pre-wrap ${!expanded && isLong ? "line-clamp-3" : ""}`}>
                {text}
            </p>
            {isLong && (
                <button
    ...
    return (
        <div className="min-h-screen bg-slate-50 p-6 relative overflow-x-hidden">
                                            ...
                                            <div className="flex-1 flex flex-col justify-between min-w-0">
                                                <div className="min-w-0">
                                                    ...
                                                    <div className="text-sm text-slate-500 mb-2 space-y-1">
                                                        <p className="break-all">
                                                            <strong>Creator:</strong> {campaign.fullName ? `${campaign.fullName} (${campaign.creator})` : campaign.creator}
                                                        </p>
                                                        <p>
                                                            <strong>Created:</strong> {campaign.createdAt ? new Date(campaign.createdAt).toLocaleString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true }) : 'N/A'}
                                                        </p>
                                                        {deadlineDate && (
                                                            <p>
                                                ...
                                                <div className="flex w-full items-center justify-between mt-6 pt-6 border-t border-slate-100">
                                                    ...
                                                    >
                                                        ...
                                                        View All Images of Requirements.
                                                    </button>
                                ...
                                {isLoadingLive ? (
                                    ...
                                    visibleLive.map((camp, idx) => (
                                        <tr key={idx} className="hover:bg-slate-50 transition">
                                            <td className="px-6 py-4 font-bold text-slate-800 truncate">{camp.name}</td>
                                            <td className="px-6 py-4 font-mono text-xs text-blue-600">
                                                ...
                                                >
                                                    <span className="truncate block w-full">{camp.campaignAddress}</span>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <Link href={`/campaign/${camp.campaignAddress}`} className="text-slate-700 hover:text-blue-600 font-bold text-xs">
                                                    View Page →
                                                </Link>
                                            </td>
                                        </tr>
            ...
            <ApproveCampaignModal
                ...
                images={campaignToApprove ? getCampaignImages(campaignToApprove) : undefined}
            />
